...
int M = 1;              // time discretization refinement
...
void prepareGeometry(LBconverter<T> const& converter, IndicatorF3D<bool,T>& indicator,
  ...
  origin[1]+=converter.getLatticeL()/2.; origin[2]+=converter.getLatticeL()/2.;
  std::vector<T> extend=superGeometry.getStatistics().getMaxPhysR(2);
  extend[1]=extend[1]-origin[1]-converter.getLatticeL()/2.; extend[2]=extend[2]-origin[2]-converter.getLatticeL()/2.;

  /// Set material number for inflow
  origin[0]=superGeometry.getStatistics().getMinPhysR(2)[0]-converter.getLatticeL(); extend[0]=2*converter.getLatticeL();
  IndicatorCuboid3D<bool,T> inflow(extend,origin);
  superGeometry.rename(2,3,inflow);

  /// Set material number for outflow
  origin[0]=superGeometry.getStatistics().getMaxPhysR(2)[0]-converter.getLatticeL(); extend[0]=2*converter.getLatticeL();
  IndicatorCuboid3D<bool,T> outflow(extend,origin);
  superGeometry.rename(2,4,outflow);

  /// Set material number for cylinder
  origin[0]=superGeometry.getStatistics().getMinPhysR(2)[0]+converter.getLatticeL(); extend[0]=(superGeometry.getStatistics().getMaxPhysR(2)[0]-superGeometry.getStatistics().getMinPhysR(2)[0])/2.;
  IndicatorCuboid3D<bool,T> cylinder(extend,origin);
...
void getResults(SuperLattice3D<T, DESCRIPTOR>& sLattice,
  ...
  if (iT%statIter==0) {
    ...
    SuperLatticePhysPressure3D<T, DESCRIPTOR> pressure(sLattice, converter);
    AnalyticalFfromSuperLatticeF3D<T, DESCRIPTOR> intpolatePressure(pressure, true);
    SuperLatticePhysDrag3D<T,DESCRIPTOR> drag(sLattice, superGeometry, 5, converter);

    std::vector<T> point1 = superGeometry.getStatistics().getCenterPhysR(5);
    std::vector<T> point2 = superGeometry.getStatistics().getCenterPhysR(5);
