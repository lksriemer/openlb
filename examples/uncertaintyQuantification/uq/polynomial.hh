// legendre_basis.hh
#ifndef LEGENDRE_BASIS_HH
#define LEGENDRE_BASIS_HH
...
static std::vector<double> computeLegendreCoefficients(size_t n) {
    if (n == 0) {
        return {1.0};  // P_0(x) = 1
    } else if (n == 1) {
...
std::vector<double> LegendreBasis::computeCoefficients(size_t n) const {
    return computeLegendreCoefficients(n);
}

// Construct the Jacobi matrix of size n for Legendre polynomials
std::vector<std::vector<double>> LegendreBasis::constructJacobiMatrix(size_t n) const {
    if (n <= 1) {
...
std::shared_ptr<Quadrature::QuadratureBase> LegendreBasis::getQuadrature(size_t nq, Quadrature::QuadratureMethod method) const {
    return std::make_shared<Quadrature::Quadrature<LegendreBasis>>(nq, method);
}


// Helper function to compute probabilist's Hermite coefficients recursively
static std::vector<double> computeHermiteCoefficients(size_t n) {
    // Base cases
    if (n == 0) {
        return {1.0};  // He_0(x) = 1
    } else if (n == 1) {
        ...
        std::vector<double> Hn_minus_two = {1.0};        // He_0(x)
        std::vector<double> Hn_minus_one = {0.0, 1.0};   // He_1(x)
        std::vector<double> Hn;  // To store coefficients of He_n(x)
...
std::vector<double> HermiteBasis::computeCoefficients(size_t n) const {
    return computeHermiteCoefficients(n);
}

// Construct the Jacobi matrix of size n for Hermite polynomials
std::vector<std::vector<double>> HermiteBasis::constructJacobiMatrix(size_t n) const {
    if (n <= 1) {
        throw std::invalid_argument("The order (n) must be greater than 1.");
    }
    ...
    // std::cout << "Jacobi matrix for Hermite polynomials:" << std::endl;
    // for (size_t i = 0; i < n; ++i) {
...
std::shared_ptr<Quadrature::QuadratureBase> HermiteBasis::getQuadrature(size_t nq, Quadrature::QuadratureMethod method) const {
    return std::make_shared<Quadrature::Quadrature<HermiteBasis>>(nq, method);
}


} // namespace Polynomials

#endif // LEGENDRE_BASIS_HH