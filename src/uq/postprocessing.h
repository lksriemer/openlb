...
void readData(
    ...
    std::string fileName =
        singleton::directories().getVtkOutDir() + "data/" +
    ...
    data.assign(info.nodeCount, std::vector<std::vector<T>>(
        info.size, std::vector<T>(samples_number)));
...
/**
  ...
  */
template <typename T, typename DESCRIPTOR>
void computeMeanAndStdAndWriteVTI(
    UncertaintyQuantification<T>& uq,
    const std::string& uqFolder,
    const std::string& name,
    const std::string& dataName,
    ...
    for (size_t iter = 0; iter < numIterations; ++iter) {
        ...
        for (int rank = 0; rank < noOfCuboids; ++rank) {
            ...
            SpecialAnalyticalFfromBlockF<T, T, D> meanField(
                blockDataFMean,
                cuboidDecomposition.get(rank),
                info.spacing,
                /*scale=*/1.0
            );
            SpecialAnalyticalFfromBlockF<T, T, D> stdField(
                blockDataFStd,
                cuboidDecomposition.get(rank),
                info.spacing,
                /*scale=*/1.0
            ...
            for (int iBalancer = 0; iBalancer < sGeometry.getLoadBalancer().size(); ++iBalancer) {
                sLattice.getBlock(iBalancer).template defineField<descriptors::VELOCITY>(
                    materialIndicator->getBlockIndicatorF(iBalancer),
                    meanField
                );
        ...
        SuperLatticePhysField<T, DESCRIPTOR, descriptors::VELOCITY> meanPhysField(
            sLattice,
            /*scale=*/1.0,
            dataName + "Mean"
        );
        ...
        SuperLatticePhysField<T, DESCRIPTOR, descriptors::VELOCITY2> stdPhysField(
            sLattice,
            /*scale=*/1.0,
            dataName + "Std"
        );
