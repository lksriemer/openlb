...
template<typename T>
std::vector<std::vector<T>> LegendreBasis<T>::constructJacobiMatrix(std::size_t n) const {
    if (n <= 1) {
...
template<typename T>
std::shared_ptr<Quadrature::QuadratureBase<T>> LegendreBasis<T>::getQuadrature(std::size_t nq, Quadrature::QuadratureMethod method) const {
...
template<typename T>
std::vector<std::vector<T>> HermiteBasis<T>::constructJacobiMatrix(std::size_t n) const {
    if (n <= 1) {
        throw std::invalid_argument("The order (n) must be greater than 1.");
    }
...
template<typename T>
std::shared_ptr<Quadrature::QuadratureBase<T>> HermiteBasis<T>::getQuadrature(std::size_t nq, Quadrature::QuadratureMethod method) const {
