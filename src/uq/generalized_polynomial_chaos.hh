...
// Constructor
template<typename T>
GeneralizedPolynomialChaos<T>::GeneralizedPolynomialChaos(std::size_t _order,
                                                       std::size_t _nq,
...
// Helper functions
template<typename T>
void GeneralizedPolynomialChaos<T>::calculateMultiIndices(std::size_t d, std::size_t n, std::vector<std::vector<std::size_t>>& indices) {

    std::vector<std::size_t> index(d, 0);

    std::function<void(std::size_t, std::size_t, std::size_t)> recursiveFunction = [&](std::size_t pos, std::size_t sum, std::size_t maxOrder) {
        if (pos == d - 1) {
...
template<typename T>
std::vector<std::size_t> GeneralizedPolynomialChaos<T>::findIndex(std::size_t idx, std::size_t dimension, std::size_t nq) {
    if (dimension == 1) {
        return {idx};
    }
...
template<typename T>
std::shared_ptr<Polynomials::PolynomialBasis<T>> GeneralizedPolynomialChaos<T>::getPolynomialBasis(std::size_t dimension) const {
