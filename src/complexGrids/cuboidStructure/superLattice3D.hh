...
SuperLattice3D<T,Lattice>::SuperLattice3D (
  CuboidGeometry3D<T>& cGeometry, int overlapBC)
  :_cGeometry(cGeometry),_commStream(*this),_commBC(*this) {
    ...
    for (int iC=0; iC<_load.size(); iC++) {
        BlockLatticeView3D<T,Lattice>lattice( _blockLattices[iC], 
            _overlap, _blockLattices[iC].getNx() - _overlap - 1 ,
            _overlap, _blockLattices[iC].getNy() - _overlap - 1,
            _overlap, _blockLattices[iC].getNz() - _overlap - 1);
...
void SuperLattice3D<T,Lattice>::defineDynamics (
         T x0, T x1, T y0, T y1, T z0, T z1, Dynamics<T,Lattice>* dynamics ) {
    ...
    for (int iC=0; iC<_load.size(); iC++) {
        if (_cGeometry.get_cuboid(_load.glob(iC)).checkInters( x0, x1, y0, y1, z0, z1,
            ...
            _blockLattices[iC].defineDynamics(locX0, locX1, locY0, locY1, locZ0, locZ1, dynamics);
        }
    }
}

template<typename T, template<typename U> class Lattice>
void SuperLattice3D<T,Lattice>::defineRhoU (
         T x0, T x1, T y0, T y1, T z0, T z1, T rho, const T u[Lattice<T>::d] ) {
    ...
    for (int iC=0; iC<_load.size(); iC++) {
        if (_cGeometry.get_cuboid(_load.glob(iC)).checkInters( x0, x1, y0, y1, z0, z1,
            ...
            for (int iX=locX0; iX<=locX1; iX++) {
                for (int iY=locY0; iY<=locY1; iY++) {
                    for (int iZ=locZ0; iZ<=locZ1; iZ++) {
                        _blockLattices[iC].get(iX,iY,iZ).iniEquilibrium(rho, u);
                    }
                }
            }
        }
...
void SuperLattice3D<T,Lattice>::stream () {
    ...
    for (int iC=0; iC<_load.size(); iC++) {
        _blockLattices[iC].stream(_overlap-1, _blockLattices[iC].getNx() - _overlap,
                              ...
                              _overlap-1, _blockLattices[iC].getNz() - _overlap);
    }
    if (_commBC_on) {
        _commBC.send();
        _commBC.receive();
...
void SuperLattice3D<T,Lattice>::collideAndStream () {
    ...
    for (int iC=0; iC<_load.size(); iC++) {
        _blockLattices[iC].bulkCollideAndStream(_overlap+1, _blockLattices[iC].getNx() - _overlap-2,
                              ...
                              _overlap+1, _blockLattices[iC].getNz() - _overlap-2);
    }
    ...
    for (int iC=0; iC<_load.size(); iC++) {
        ...
        _blockLattices[iC].boundaryStream(0,x1,0,y1,0,z1,
           _overlap-1,x1-_overlap+1,    _overlap-1,_overlap,         _overlap-1,z1-_overlap+1);
        _blockLattices[iC].boundaryStream(0,x1,0,y1,0,z1,
           _overlap-1,x1-_overlap+1,     y1-_overlap,y1-_overlap+1,  _overlap-1,z1-_overlap+1);
        _blockLattices[iC].boundaryStream(0,x1,0,y1,0,z1,
           _overlap-1, _overlap,         _overlap+1, y1-_overlap-1,  _overlap-1,z1-_overlap+1);
        _blockLattices[iC].boundaryStream(0,x1,0,y1,0,z1,
           x1-_overlap,x1-_overlap+1,    _overlap+1, y1-_overlap-1,  _overlap-1,z1-_overlap+1);
        _blockLattices[iC].boundaryStream(0,x1,0,y1,0,z1,
           _overlap+1, x1-_overlap-1,      _overlap+1, y1-_overlap-1,    _overlap-1,_overlap);
        _blockLattices[iC].boundaryStream(0,x1,0,y1,0,z1,
           _overlap+1, x1-_overlap-1,      _overlap+1, y1-_overlap-1,    z1-_overlap,z1-_overlap+1);
    }
