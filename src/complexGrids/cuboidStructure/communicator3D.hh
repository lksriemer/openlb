...
void Communicator3D<T,Lattice>::init_nh() {
    ...
    for (int iC=0; iC<_nC; iC++) {
        CuboidNeighbourhood3D<T,Lattice> nh(_sLattice,iC);
        _nh.push_back(nh);
    }
}
...
void Communicator3D<T,Lattice>::add_cell(int iC, T globX, T globY, T globZ) {
    ...
    _nh[iC].add_inCell(globX, globY, globZ);
}
...
template<typename T, template<typename U> class Lattice>
void Communicator3D<T,Lattice>::sync() {

    #ifdef PARALLEL_MODE_MPI
        int rank;
        int nCells;
        T c_globX, c_globY, c_globZ;
        for (int iC=0; iC<_nC; iC++) {
            ...
            for (int i=0; i<nCells; i++) {
                if (singleton::mpi().getRank()==rank) {
                    ...
                    c_globZ = _nh[iC].get_inCell(i).globZ;
                }
                singleton::mpi().bCast(&c_globX, 1, rank);
                singleton::mpi().bCast(&c_globY, 1, rank);
                singleton::mpi().bCast(&c_globZ, 1, rank);
                if (singleton::mpi().getRank()!=rank) {
                    _nh[iC].add_inCell(c_globX, c_globY, c_globZ);
                }
            }
        }
    #endif
