...
public:
    ...
    typedef descriptors::ExternalFieldArray <
                T,
                typename Lattice<T>::ExternalField
            > External;
public:
    /// Default constructor.
    Cell();
...
public:
     ...
     * object), unless that the dynamics object is not copied.
     */
    ...
    void collide() {
        ...
        dynamics->collide(*this);
    }
    /// Apply LB collision with fixed velocity to the cell.
    void staticCollide(const T u[Lattice<T>::d]) {
        OLB_PRECONDITION( dynamics );
        dynamics->staticCollide(*this, u);
    }
