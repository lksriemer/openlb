...
BasicDynamics<T,Lattice>::BasicDynamics(
        Momenta<T,Lattice>& momenta_,
        LatticeStatistics<T>& statistics_)
    : momenta(momenta_), statistics(statistics_)
{ }
...
BGKdynamics<T,Lattice>::BGKdynamics (
        ...
        LatticeStatistics<T>& statistics_ )
    : BasicDynamics<T,Lattice>(momenta_, statistics_),
      omega(omega_)
{ }
...
template<typename T, template<typename U> class Lattice>
void BGKdynamics<T,Lattice>::collide(Cell<T,Lattice>& cell) {
    T rho, u[Lattice<T>::d];
    this->momenta.computeRhoU(cell, rho, u);
    T uSqr = lbHelpers<T,Lattice>::bgkCollision(cell, rho, u, omega);
    if (cell.takesStatistics()) {
        this->statistics.gatherStats(rho, uSqr);
    }
}

template<typename T, template<typename U> class Lattice>
void BGKdynamics<T,Lattice>::staticCollide(Cell<T,Lattice>& cell,
                                           const T u[Lattice<T>::d] ) 
{
    T rho;
...
template<typename T, template<typename U> class Lattice>
void ConstRhoBGKdynamics<T,Lattice>::collide(Cell<T,Lattice>& cell) {
    T rho, u[Lattice<T>::d];
    this->momenta.computeRhoU(cell, rho, u);

    T deltaRho = (T)1 - (this->statistics).getAverageRho();
    T ratioRho = (T)1 + deltaRho/rho;

    T uSqr = lbHelpers<T,Lattice>::constRhoBgkCollision (
                cell, rho, u, ratioRho, omega );
    if (cell.takesStatistics()) {
        this->statistics.gatherStats(rho+deltaRho, uSqr);
    }
}

template<typename T, template<typename U> class Lattice>
void ConstRhoBGKdynamics<T,Lattice>::staticCollide(Cell<T,Lattice>& cell,
                                           const T u[Lattice<T>::d] )
{
...
template<typename T, template<typename U> class Lattice>
void IncBGKdynamics<T,Lattice>::collide(Cell<T,Lattice>& cell) {
    T rho = this->momenta.computeRho(cell);
...
template<typename T, template<typename U> class Lattice>
void IncBGKdynamics<T,Lattice>::staticCollide(Cell<T,Lattice>& cell,
...
template<typename T, template<typename U> class Lattice>
void RLBdynamics<T,Lattice>::collide(Cell<T,Lattice>& cell) {
...
template<typename T, template<typename U> class Lattice>
void RLBdynamics<T,Lattice>::staticCollide(Cell<T,Lattice>& cell,
                                           const T u[Lattice<T>::d])
{
...
CombinedRLBdynamics<T,Lattice,Dynamics>::CombinedRLBdynamics (
        ...
        LatticeStatistics<T>& statistics_ )
    : BasicDynamics<T,Lattice>(momenta_, statistics_),
      boundaryDynamics(omega_, momenta_, statistics_)
{ }

template<typename T, template<typename U> class Lattice, typename Dynamics>
CombinedRLBdynamics<T,Lattice,Dynamics>* CombinedRLBdynamics<T,Lattice, Dynamics>::clone() const
{
    return new CombinedRLBdynamics<T,Lattice,Dynamics>(*this);
}
 
template<typename T, template<typename U> class Lattice, typename Dynamics>
void CombinedRLBdynamics<T,Lattice,Dynamics>::collide(Cell<T,Lattice>& cell)
{
    typedef lbHelpers<T,Lattice> Helpers;
    typedef Lattice<T> L;
    ...
    {
        cell[iPop] = Helpers::equilibrium(iPop,rho,u,uSqr) +
                     firstOrderLbHelpers<T,Lattice>::fromPiToFneq(iPop, pi);
    }

    boundaryDynamics.collide(cell);
        ...
        const T u[Lattice<T>::d] )
{
    ...
    boundaryDynamics.staticCollide(cell, u);
}
...
template<typename T, template<typename U> class Lattice>
void ForcedBGKdynamics<T,Lattice>::collide(Cell<T,Lattice>& cell) {
    T rho, u[Lattice<T>::d];
    this->momenta.computeRhoU(cell, rho, u);
...
template<typename T, template<typename U> class Lattice>
void ForcedBGKdynamics<T,Lattice>::staticCollide(Cell<T,Lattice>& cell,
                                           const T u[Lattice<T>::d] ) 
{
...
template<typename T, template<typename U> class Lattice>
void D3Q13dynamics<T,Lattice>::collide(Cell<T,Lattice>& cell) {
    T rho, u[Lattice<T>::d];
    this->momenta.computeRhoU(cell, rho, u);
...
template<typename T, template<typename U> class Lattice>
void D3Q13dynamics<T,Lattice>::staticCollide(Cell<T,Lattice>& cell,
                                           const T u[Lattice<T>::d])
{
    // NOT YET IMPLEMENTED
...
template<typename T, template<typename U> class Lattice>
void BounceBack<T,Lattice>::collide(Cell<T,Lattice>& cell) {
...
template<typename T, template<typename U> class Lattice>
void NoDynamics<T,Lattice>::collide(Cell<T,Lattice>& cell) { }
