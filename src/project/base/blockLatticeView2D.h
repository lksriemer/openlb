...
/// A rectangular extract from a given BlockLattice2D.
/** Attention:
...
public:
    BlockLatticeView2D(BlockLattice2D<T,Lattice>& originalLattice_);
    BlockLatticeView2D(BlockLattice2D<T,Lattice>& originalLattice_,
    ...
    virtual void stream (
                int x0_, int x1_, int y0_, int y1_, bool periodic=false );
...
private:
    BlockLattice2D<T,Lattice>  *originalLattice;
    int                        x0, y0;
    int                        nx,ny;
};
