...
struct Reductor {
    ...
    virtual void startNewSubscription() =0;
    virtual void performReduction() =0;
};
...
public:
    ...
    virtual PostProcessor2D<T,Lattice>* generate (
                BlockLattice2D<T,Lattice>& blockLattice ) const =0;
...
public:
    ...
    virtual PostProcessor3D<T,Lattice>* generate (
                BlockLattice3D<T,Lattice>& blockLattice ) const =0;
...
public:
    ...
    ~LatticeStatistics();
    void reset();
    void reset(T average_rho_, T average_energy_, T maxU_,
               int numCells_);
