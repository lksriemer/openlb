...
BlockLatticeView2D<T,Lattice>::BlockLatticeView2D (
        BlockLattice2D<T,Lattice>& originalLattice_ )
...
template<typename T, template<typename U> class Lattice>
void BlockLatticeView2D<T,Lattice>::stream (
        int x0_, int x1_, int y0_, int y1_, bool periodic )
{
    originalLattice->stream( x0_+x0, x1_+x0,
                             y0_+y0, y1_+y0, periodic );
}

template<typename T, template<typename U> class Lattice>
void BlockLatticeView2D<T,Lattice>::stream(bool periodic) {
    originalLattice->stream( x0, x0+nx-1, y0, y0+ny-1, periodic);
    postProcess();
}

template<typename T, template<typename U> class Lattice>
void BlockLatticeView2D<T,Lattice>::collideAndStream (
        int x0_, int x1_, int y0_, int y1_, bool periodic )
{
...
void BlockLatticeView2D<T,Lattice>::collideAndStream(bool periodic) {
    originalLattice->collideAndStream( x0, x0+nx-1, y0, y0+ny-1, periodic);
    postProcess();
}
...
template<typename T, template<typename U> class Lattice>
LatticeStatistics<T>& BlockLatticeView2D<T,Lattice>::
    getStatistics()
{
...
template<typename T, template<typename U> class Lattice>
LatticeStatistics<T> const& BlockLatticeView2D<T,Lattice>::
    getStatistics() const
{
    return originalLattice->getStatistics();
}
