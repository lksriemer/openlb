...
public:
  ...
  virtual std::vector<T> getPhysR(int iX, int iY, int iZ) const = 0;
  /// Transforms lattice to physical coordinates (wrapped from cuboid geometry)
  ...
  virtual void rename(int fromM, int toM, IndicatorF3D<bool,T>& condition);
  ...
  virtual void rename(int fromM, int toM, int fluidM, IndicatorF3D<bool,T>& condition, std::vector<int> discreteNormal);
  ...
  virtual void rename(int fromM, int toM, int fluidM, IndicatorF3D<bool,T>& condition);
