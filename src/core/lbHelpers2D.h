...
template<typename T>
struct lbHelpers<T, descriptors::D2Q9Descriptor> {
    ...
    static T incEquilibrium( int iPop, const T j[2], const T jSqr,
                             const T pressure ) {
        typedef descriptors::D2Q9Descriptor<T> L;
    ...
    static void computeFneq (
            Cell<T,descriptors::D2Q9Descriptor> const& cell,
            T fNeq[9], T rho, const T u[2] )
    {
    ...
    static T constRhoBgkCollision (
            Cell<T,descriptors::D2Q9Descriptor>& cell,
            T rho, const T u[2], T ratioRho, T omega)
    {
        const T uSqr = util::normSqr<T,descriptors::D2Q9Descriptor<T>::d>(u);
        for (int iPop=0; iPop < descriptors::D2Q9Descriptor<T>::q; ++iPop) {
            T feq = lbHelpers<T,descriptors::D2Q9Descriptor>::
                         equilibrium(iPop, rho, u, uSqr );
            cell[iPop] =
              ratioRho*(feq+descriptors::D2Q9Descriptor<T>::t[iPop])
    ...
    static void computeJ (
        Cell<T,descriptors::D2Q9Descriptor> const& cell, T j[2] )
    {
    ...
    static void computeStress (
        Cell<T,descriptors::D2Q9Descriptor> const& cell, T rho,
        const T u[2], T pi[3] )
    {
        typedef descriptors::D2Q9Descriptor<T> L;
    ...
    static void modifyVelocity (
            Cell<T,descriptors::D2Q9Descriptor>& cell, const T newU[2] )
    {
        T rho, oldU[2];
...
template<typename T>
struct lbHelpers<T, descriptors::ForcedD2Q9Descriptor> {

    static T equilibrium( int iPop, T rho, const T u[2], T uSqr ) {
        typedef descriptors::ForcedD2Q9Descriptor<T> L;
        T c_u = L::c[iPop][0]*u[0] + L::c[iPop][1]*u[1];
        return rho * L::t[iPop] * (
                   1. + 3.*c_u + 4.5*c_u*c_u - 1.5*uSqr)
               - L::t[iPop];
    ...
    static void computeFneq (
            Cell<T,descriptors::ForcedD2Q9Descriptor> const& cell,
            T fNeq[2], T rho, const T u[2] )
    {
        const T uSqr = u[0]*u[0] + u[1]*u[1];
        for (int iPop=0; iPop < 9; ++iPop) {
            fNeq[iPop] = cell[iPop] - equilibrium(iPop, rho, u, uSqr);
    ...
    static T constRhoBgkCollision (
            ...
            T rho, const T u[2], T ratioRho, T omega)
    {
        const T uSqr = util::normSqr<T,descriptors::ForcedD2Q9Descriptor<T>::d>(u);
        for (int iPop=0; iPop < descriptors::ForcedD2Q9Descriptor<T>::q; ++iPop) {
            T feq = lbHelpers<T,descriptors::ForcedD2Q9Descriptor>::
                         equilibrium(iPop, rho, u, uSqr );
            cell[iPop] =
              ratioRho*(feq+descriptors::ForcedD2Q9Descriptor<T>::t[iPop])
              -descriptors::ForcedD2Q9Descriptor<T>::t[iPop] +
                  ((T)1-omega)*(cell[iPop]-feq);
        }
        return uSqr;
    }
    ...
    static void computeJ (
        Cell<T,descriptors::ForcedD2Q9Descriptor> const& cell, T j[2] )
    {
        T lineX_P1, lineX_M1, lineY_P1, lineY_M1;
    ...
    static void computeStress (
        Cell<T,descriptors::ForcedD2Q9Descriptor> const& cell, T rho,
        const T u[2], T pi[3] )
    {
        typedef descriptors::ForcedD2Q9Descriptor<T> L;
        // Workaround for Intel(r) compiler 9.1;
    ...
    static void modifyVelocity (
            Cell<T,descriptors::ForcedD2Q9Descriptor>& cell, const T newU[2] )
    {
        T rho, oldU[2];
        computeRhoU(cell, rho, oldU);
        const T oldUSqr = util::normSqr<T,2>(oldU);
...
};  //struct lbHelpers<ForcedD2Q9Descriptor>

// Efficient specialization for D2Q9 lattice with force
template<typename T>
struct externalFieldHelpers<T, descriptors::ForcedD2Q9Descriptor> {
    static void addExternalForce(
                    Cell<T,descriptors::ForcedD2Q9Descriptor>& cell,
                    const T u[descriptors::ForcedD2Q9Descriptor<T>::d],
                    T omega,
                    T amplitude=(T)1)
    {
