...
template<typename T, template<typename U> class Lattice>
void BlockStatistics3D<T,Lattice>::computeStrainRateFieldFromStress() const {
    ...
    for (int iX=0; iX<velField.getNx(); ++iX) {
        for (int iY=0; iY<velField.getNy(); ++iY) {
            for (int iZ=0; iZ<velField.getNz(); ++iZ) {
                block.get(iX,iY,iZ).computeStress(stressField.get(iX,iY,iZ));
                T omega = block.get(iX,iY,iZ).getDynamics()->getOmega();
                for (int iPi=0; iPi<6; ++iPi) {
                    stressField.get(iX,iY,iZ)[iPi] *= -omega / (T)2 * Lattice<T>::invCs2;
                }
            }
        }
    }
