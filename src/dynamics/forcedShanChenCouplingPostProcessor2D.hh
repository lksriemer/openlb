    ...
    for (int iX=newX0; iX<=newX1; ++iX) {
      for (int iY=newY0; iY<=newY1; ++iY) {
        ...
        for (int iPop = 0; iPop < L::q; ++iPop) {
          ...
          T blockRho   = PsiIsRho(rhoField1.get(nextX-offsetX, nextY-offsetY));//rho0[0];
          T partnerRho = PsiIsRho(rhoField2.get(nextX-offsetX, nextY-offsetY));///rho0[1];
          for (int iD = 0; iD < L::d; ++iD) {
            rhoBlockContribution[iD]   += PsiIsRho(rhoField2.get(iX-offsetX, iY-offsetY)) * blockRho * L::c[iPop][iD]* L::t[iPop];
            rhoPartnerContribution[iD] += PsiIsRho(rhoField1.get(iX-offsetX, iY-offsetY)) * partnerRho * L::c[iPop][iD]* L::t[iPop];
          }
        }
        ...
        for (int iD = 0; iD < L::d; ++iD) {
          blockU[iD] = uTot[iD] + 1./blockOmega *
                       (blockForce[iD] - G*rhoPartnerContribution[iD]/rhoField1.get(iX-offsetX, iY-offsetY));
          partnerU[iD] = uTot[iD] + 1./partnerOmega *
                         (partnerForce[iD] - G*rhoBlockContribution[iD]/rhoField2.get(iX-offsetX, iY-offsetY));
        }
      }
    }
  }
