...
#ifndef FORCED_SHAN_CHEN_SINGLE_COMPONENT_COUPLING_POST_PROCESSOR_2D_H
#define FORCED_SHAN_CHEN_SINGLE_COMPONENT_COUPLING_POST_PROCESSOR_2D_H
...
class ForcedShanChenSingleComponentCouplingPostProcessor2D : public LocalPostProcessor2D<T,Lattice> {
public:
...
template<typename T, template<typename U> class Lattice>
class ForcedShanChenSingleComponentCouplingGenerator2D : public LatticeCouplingGenerator2D<T,Lattice> {
public:
  ForcedShanChenSingleComponentCouplingGenerator2D(int x0_, int x1_, int y0_, int y1_, T G_, std::vector<T> rho0_);
