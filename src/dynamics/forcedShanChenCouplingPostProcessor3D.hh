      ...
      for (int iY=newY0; iY<=newY1; ++iY) {
        for (int iZ=newZ0; iZ<=newZ1; ++iZ) {
          ...
          for (int iPop = 0; iPop < L::q; ++iPop) {
            ...
            T blockRho   = PsiIsRho(rhoField1.get(nextX-offsetX, nextY-offsetY, nextZ-offsetZ));
            T partnerRho = PsiIsRho(rhoField2.get(nextX-offsetX, nextY-offsetY, nextZ-offsetZ));
            for (int iD = 0; iD < L::d; ++iD) {
              rhoBlockContribution[iD]   += PsiIsRho(rhoField2.get(iX-offsetX, iY-offsetY, iZ-offsetZ)) * blockRho * L::c[iPop][iD]* L::t[iPop];
              rhoPartnerContribution[iD] += PsiIsRho(rhoField1.get(iX-offsetX, iY-offsetY, iZ-offsetZ)) * partnerRho * L::c[iPop][iD]* L::t[iPop];
            }
          }
