...
#ifndef FORCED_SHAN_CHEN_SINGLE_COMPONENT_COUPLING_POST_PROCESSOR_3D_HH
#define FORCED_SHAN_CHEN_SINGLE_COMPONENT_COUPLING_POST_PROCESSOR_3D_HH

#include "forcedShanChenSingleComponentCouplingPostProcessor3D.h"
...
////////  ForcedShanChenSingleComponentCouplingPostProcessor3D ///////////////////////////////////
...
ForcedShanChenSingleComponentCouplingPostProcessor3D <T,Lattice>::
ForcedShanChenSingleComponentCouplingPostProcessor3D(int x0_, int x1_, int y0_, int y1_, int z0_, int z1_,
...
template<typename T, template<typename U> class Lattice>
void ForcedShanChenSingleComponentCouplingPostProcessor3D<T,Lattice>::
      ...
      for (int iY=newY0; iY<=newY1; ++iY) {
        for (int iZ=newZ0; iZ<=newZ1; ++iZ) {
          ...
          for (int iPop = 0; iPop < L::q; ++iPop) {
            ...
            T blockRho   = ShanChen(rhoField1.get(nextX-offsetX, nextY-offsetY, nextZ-offsetZ));
            for (int iD = 0; iD < L::d; ++iD) {
              rhoBlockContribution[iD]   += ShanChen(rhoField1.get(iX-offsetX, iY-offsetY, iZ-offsetZ)) * blockRho * L::c[iPop][iD]* L::t[iPop];
            }
          }
          ...
          for (int iD = 0; iD < L::d; ++iD) {
            blockU[iD] = uTot[iD] + 1./blockOmega * (blockForce[iD] - G*rhoBlockContribution[iD]/rhoField1.get(iX-offsetX, iY-offsetY, iZ-offsetZ));
          }
        }
      }
    }
...
template<typename T, template<typename U> class Lattice>
ForcedShanChenSingleComponentCouplingGenerator3D<T,Lattice>::ForcedShanChenSingleComponentCouplingGenerator3D (
...
template<typename T, template<typename U> class Lattice>
PostProcessor3D<T,Lattice>* ForcedShanChenSingleComponentCouplingGenerator3D<T,Lattice>::generate (
  std::vector<SpatiallyExtendedObject3D*> partners) const
{
  return new ForcedShanChenSingleComponentCouplingPostProcessor3D<T,Lattice>(
           this->x0,this->x1,this->y0,this->y1,this->z0,this->z1, G, rho0, partners);
}

template<typename T, template<typename U> class Lattice>
LatticeCouplingGenerator3D<T,Lattice>* ForcedShanChenSingleComponentCouplingGenerator3D<T,Lattice>::clone() const {
  return new ForcedShanChenSingleComponentCouplingGenerator3D<T,Lattice>(*this);
}
