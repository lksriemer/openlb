...
SuperMin3D<T,DESCRIPTOR>::SuperMin3D(SuperF3D<T>& f,
                                     SuperGeometry3D<T>& superGeometry, const int material)
  : SuperF3D<T>(f.getSuperStructure(),f.getTargetDim()),
...
template <typename T, template <typename U> class DESCRIPTOR>
bool SuperMin3D<T,DESCRIPTOR>::operator() (T output[], const int input[])
{
...
template <typename T, template <typename U> class DESCRIPTOR>
SuperMax3D<T,DESCRIPTOR>::SuperMax3D(SuperF3D<T>& f,
                                     SuperGeometry3D<T>& superGeometry, const int material)
  : SuperF3D<T>(f.getSuperStructure(),f.getTargetDim()),
    _f(f), _superGeometry(superGeometry), _material(material)
{
...
template <typename T, template <typename U> class DESCRIPTOR>
bool SuperMax3D<T,DESCRIPTOR>::operator() (T output[], const int input[])
{
  _f.getSuperStructure().communicate();
  CuboidGeometry3D<T>& cGeometry = _f.getSuperStructure().getCuboidGeometry();
  LoadBalancer<T>& load = _f.getSuperStructure().getLoadBalancer();
  ...
  for (int i = 0; i < this->getTargetDim(); ++i) {
    output[i] = std::numeric_limits<T>::min();
    for (int iC = 0; iC < load.size(); ++iC) {
      int nX = cGeometry.get(load.glob(iC)).getNx();
      int nY = cGeometry.get(load.glob(iC)).getNy();
      int nZ = cGeometry.get(load.glob(iC)).getNz();
...
template <typename T, template <typename U> class DESCRIPTOR>
SuperSum3D<T,DESCRIPTOR>::SuperSum3D(SuperF3D<T>& f,
                                     SuperGeometry3D<T>& superGeometry, const int material)
  : SuperF3D<T>(f.getSuperStructure(),f.getTargetDim()+1), _f(f),
...
template <typename T, template <typename U> class DESCRIPTOR>
bool SuperSum3D<T,DESCRIPTOR>::operator() (T output[], const int input[])
{
  _f.getSuperStructure().communicate();
  CuboidGeometry3D<T>& cGeometry = _f.getSuperStructure().getCuboidGeometry();
  LoadBalancer<T>& load = _f.getSuperStructure().getLoadBalancer();
...
template <typename T, template <typename U> class DESCRIPTOR>
SuperSumIndicator3D<T,DESCRIPTOR>::SuperSumIndicator3D(SuperF3D<T>& f,
    SuperGeometry3D<T>& superGeometry, SmoothIndicatorSphere3D<T,T>& indicator)
  : SuperF3D<T>(f.getSuperStructure(),f.getTargetDim()+1),
...
template <typename T, template <typename U> class DESCRIPTOR>
bool SuperSumIndicator3D<T,DESCRIPTOR>::operator() (T output[], const int input[])
{
  _f.getSuperStructure().communicate();
  CuboidGeometry3D<T>& cGeometry = _f.getSuperStructure().getCuboidGeometry();
  LoadBalancer<T>& load = _f.getSuperStructure().getLoadBalancer();
...
template <typename T, template <typename U> class DESCRIPTOR>
SuperAverage3D<T,DESCRIPTOR>::SuperAverage3D(SuperF3D<T>& f,
    SuperGeometry3D<T>& superGeometry, const int material)
  : SuperF3D<T>(f.getSuperStructure(),f.getTargetDim()+1),
    _f(f), _superGeometry(superGeometry), _material(material)
{
...
template <typename T, template <typename U> class DESCRIPTOR>
bool SuperAverage3D<T,DESCRIPTOR>::operator() (T output[], const int input[])
{
  _f.getSuperStructure().communicate();
  CuboidGeometry3D<T>& cGeometry = _f.getSuperStructure().getCuboidGeometry();
  LoadBalancer<T>& load = _f.getSuperStructure().getLoadBalancer();
...
template <typename T, template <typename U> class DESCRIPTOR>
SuperIntegral3D<T,DESCRIPTOR>::SuperIntegral3D(SuperF3D<T>& f,
    SuperGeometry3D<T>& superGeometry, const int material)
  : SuperF3D<T>(f.getSuperStructure(),f.getTargetDim()),
    _f(f), _superGeometry(superGeometry), _material(material)
{
...
template <typename T, template <typename U> class DESCRIPTOR>
bool SuperIntegral3D<T,DESCRIPTOR>::operator() (T output[], const int input[])
{
  _f.getSuperStructure().communicate();
  CuboidGeometry3D<T>& cGeometry = _f.getSuperStructure().getCuboidGeometry();
  LoadBalancer<T>& load = _f.getSuperStructure().getLoadBalancer();
...
template <typename T, template <typename U> class DESCRIPTOR>
SuperL1Norm3D<T,DESCRIPTOR>::SuperL1Norm3D(SuperF3D<T>& f,
    SuperGeometry3D<T>& superGeometry, const int material)
  : SuperF3D<T>(f.getSuperStructure(),1),
    _f(f), _superGeometry(superGeometry), _material(material)
{
...
template <typename T, template <typename U> class DESCRIPTOR>
bool SuperL1Norm3D<T,DESCRIPTOR>::operator() (T output[], const int input[])
{
  _f.getSuperStructure().communicate();
  CuboidGeometry3D<T>& cGeometry = _f.getSuperStructure().getCuboidGeometry();
  LoadBalancer<T>& load = _f.getSuperStructure().getLoadBalancer();
...
template <typename T, template <typename U> class DESCRIPTOR>
SuperL2Norm3D<T,DESCRIPTOR>::SuperL2Norm3D(SuperF3D<T>& f,
    SuperGeometry3D<T>& superGeometry, const int material)
  : SuperF3D<T>(f.getSuperStructure(),1), _f(f), _superGeometry(superGeometry),
    _material(material)
{
...
template <typename T, template <typename U> class DESCRIPTOR>
bool SuperL2Norm3D<T,DESCRIPTOR>::operator() (T output[], const int input[])
{
  _f.getSuperStructure().communicate();
  CuboidGeometry3D<T>& cGeometry = _f.getSuperStructure().getCuboidGeometry();
  LoadBalancer<T>& load = _f.getSuperStructure().getLoadBalancer();
...
//template <typename T, template <typename U> class DESCRIPTOR>
//SuperLpNorm3D<T,DESCRIPTOR>::SuperLpNorm3D(SuperF3D<T>& f,
//  SuperGeometry3D<T>& superGeometry, const int material, int p)
//  : SuperF3D<T>(f.getSuperStructure(),1), _f(f), _superGeometry(superGeometry),
//    _material(material), _p(p)
//{
//  this->getName() = "LpNorm("+_f.getName()+")";
//}
//
//template <typename T, template <typename U> class DESCRIPTOR>
//bool SuperLpNorm3D<T,DESCRIPTOR>::operator() (T output[], const int input[])
//{
//  _f.getSuperStructure().communicate();
//  CuboidGeometry3D<T>& cGeometry = _f.getSuperStructure().getCuboidGeometry();
//  LoadBalancer<T>& load = _f.getSuperStructure().getLoadBalancer();
...
//            _f(outputTmp,load.glob(iC),iX,iY,iZ);
//            for (int iDim = 0; iDim < _f.getTargetDim(); ++iDim) {
//              output[0] += pow(fabs(outputTmp[iDim]), _p)*weight;
//            }
//            numVoxels++;
//          }
//        }
...
template <typename T, template <typename U> class DESCRIPTOR>
SuperLpNorm3D<T,DESCRIPTOR>::SuperLpNorm3D(SuperF3D<T>& f, SuperGeometry3D<T>& geo,
    DiscreteIndicatorF3D& indicatorF, int p)
  : SuperF3D<T>(f.getSuperStructure(),1), _f(f), _superGeometry(geo),
...
template <typename T, template <typename U> class DESCRIPTOR>
SuperLpNorm3D<T,DESCRIPTOR>::SuperLpNorm3D(SuperF3D<T>& f, SuperGeometry3D<T>& geo, std::vector<int> materials, int p)
  : SuperLpNorm3D(f, geo, *(new DiscreteIndicatorMaterial3D<T> (geo, materials)), p)
{};

template <typename T, template <typename U> class DESCRIPTOR>
SuperLpNorm3D<T,DESCRIPTOR>::SuperLpNorm3D(SuperF3D<T>& f, SuperGeometry3D<T>& geo, const int material, int p)
  : SuperLpNorm3D(f, geo, std::vector<int> (1, material), p)
{};

template <typename T, template <typename U> class DESCRIPTOR>
SuperLpNorm3D<T,DESCRIPTOR>::SuperLpNorm3D(SuperF3D<T>& f, SuperGeometry3D<T>& geo, int p)
  : SuperLpNorm3D(f, geo, *(new DiscreteIndicatorTrue3D()), p)
{};
    ...
    for (int iX = 0; iX < nX; ++iX) {
      for (int iY = 0; iY < nY; ++iY) {
        for (int iZ = 0; iZ < nZ; ++iZ) {
          ...
          if (_indicatorF(outputIndicator, pointTmp)) {
            _f(outputTmp,load.glob(iC),iX,iY,iZ);
            for (int iDim = 0; iDim < _f.getTargetDim(); ++iDim) {
              output[0] += pow(fabs(outputTmp[iDim]), _p)*weight;
            }
...
template <typename T, template <typename U> class DESCRIPTOR>
SuperLinfNorm3D<T,DESCRIPTOR>::SuperLinfNorm3D(SuperF3D<T>& f,
    SuperGeometry3D<T>& superGeometry, const int material)
  : SuperF3D<T>(f.getSuperStructure(),1), _f(f), _superGeometry(superGeometry),
    _material(material)
{
...
template <typename T, template <typename U> class DESCRIPTOR>
bool SuperLinfNorm3D<T,DESCRIPTOR>::operator() (T output[], const int input[])
{
  _f.getSuperStructure().communicate();
  CuboidGeometry3D<T>& cGeometry = _f.getSuperStructure().getCuboidGeometry();
  LoadBalancer<T>& load = _f.getSuperStructure().getLoadBalancer();
...
SuperGeometryFaces3D<T>::SuperGeometryFaces3D(SuperGeometry3D<T>& superGeometry,
  ...
  : GenericF<T,int>(7,0), _superGeometry(superGeometry), _material(material),
    _converter(converter)
{
  for (int iC = 0; iC < _superGeometry.getLoadBalancer().size(); ++iC) {
    _blockGeometryFaces.push_back(new BlockGeometryFaces3D<T>(_superGeometry.getBlockGeometry(iC), _material, _converter));
  }
...
(SuperLattice3D<T,DESCRIPTOR>& sLattice, SuperGeometry3D<T>& superGeometry,
 const int material, const LBconverter<T>& converter)
  : SuperLatticePhysF3D<T,DESCRIPTOR>(sLattice,converter,3),
    ...
    _pBoundForce(this->_sLattice, _superGeometry, _material, this->_converter),
    _sumF(_pBoundForce, _superGeometry, _material),
    _factor(2. / (this->_converter.getCharRho() * this->_converter.getCharU() * this->_converter.getCharU()))
{
...
template <typename T, template <typename U> class DESCRIPTOR>
bool SuperLatticePhysDragIndicator3D<T,DESCRIPTOR>::operator() (T output[], const int input[])
{
  ...
  SuperSumIndicator3D<T,DESCRIPTOR> sumF(pBoundForce, _superGeometry, _indicator);
...
SuperLatticeFlux3D<T, DESCRIPTOR>::SuperLatticeFlux3D(SuperLatticeF3D<T, DESCRIPTOR>& f,
  ...
  : SuperLatticeF3D<T, DESCRIPTOR>(f.getSuperLattice(),5), _sg(sg), _u(u), _v(v),
    _origin(A), _rad(radius), _h(h), _analyticalF(f, false)
{
  _mat.push_back(1);
...
SuperLatticeFlux3D<T, DESCRIPTOR>::SuperLatticeFlux3D(SuperLatticeF3D<T, DESCRIPTOR>& f,
  ...
  : SuperLatticeF3D<T, DESCRIPTOR>(f.getSuperLattice(),5), _sg(sg),  _u(u), _v(v),
    _origin(A), _rad(radius), _h(h), _mat(materials), _analyticalF(f, false)
{
  _normal = (T(1)/norm(crossProduct3D(_u, _v)))*crossProduct3D(_u, _v);
  init(f);
}
...
SuperLatticeFlux3D<T, DESCRIPTOR>::SuperLatticeFlux3D(SuperLatticeF3D<T, DESCRIPTOR>& f,
  ...
  : SuperLatticeF3D<T, DESCRIPTOR>(f.getSuperLattice(),5), _sg(sg), _u(3, 0.),
    _v(3, 0.), _origin(A), _normal(n), _rad(radius), _h(h), _analyticalF(f, false)
{
  _mat.push_back(1);
  init(f);
}
...
SuperLatticeFlux3D<T, DESCRIPTOR>::SuperLatticeFlux3D(SuperLatticeF3D<T, DESCRIPTOR>& f,
    ...
    T radius, T h)
  : SuperLatticeF3D<T, DESCRIPTOR>(f.getSuperLattice(),5), _sg(sg), _u(3, 0.),
    _v(3, 0.), _origin(A), _normal(n), _rad(radius), _h(h), _mat(materials), _analyticalF(f, false)
{
  init(f);
}
...
SuperLatticeFlux3D<T, DESCRIPTOR>::SuperLatticeFlux3D(SuperLatticeF3D<T, DESCRIPTOR>& f,
  ...
  : SuperLatticeF3D<T, DESCRIPTOR>(f.getSuperLattice(),5), _sg(sg), _u(3, 0.),
    _v(3, 0.), _origin(circle.getCenter()), _normal(circle.getNormal()),
    _rad(circle.getRadius()), _h(h), _vox(0), _analyticalF(f, false)
{
  _mat.push_back(1);
  init(f);
}
...
template<typename T, template<typename U> class DESCRIPTOR>
SuperLatticeFlux3D<T, DESCRIPTOR>::SuperLatticeFlux3D(SuperLatticeF3D<T, DESCRIPTOR>& f,
    SuperGeometry3D<T>& sg, IndicatorCircle3D<T>& circle, std::list<int> materials, T h)
  : SuperLatticeF3D<T, DESCRIPTOR>(f.getSuperLattice(),5), _sg(sg), _u(3, 0.),
    _v(3, 0.), _origin(circle.getCenter()), _normal(circle.getNormal()),
    _rad(circle.getRadius()), _h(h), _vox(0), _mat(materials), _analyticalF(f, false)
{
  init(f);
}
    ...
    int yDir, int xStart, int yStart)
{
  ...
  while ( pow(i*_u[0]+j*_v[0],2) + pow(i*_u[1]+j*_v[1],2) + pow(i*_u[2]+j*_v[2],2) < pow(_rad,2) ) {
    while ( pow(i*_u[0]+j*_v[0],2) + pow(i*_u[1]+j*_v[1],2) + pow(i*_u[2]+j*_v[2],2) < pow(_rad,2) ) {
      T pos[3] = {_origin[0]+i*_u[0]+j*_v[0], _origin[1]+i*_u[1]+j*_v[1], _origin[2]+i*_u[2]+j*_v[2]};
...
template<typename T, template<typename U> class DESCRIPTOR>
bool SuperLatticePhysPressureFlux3D<T, DESCRIPTOR>::operator()(T output[], const int input[])
{
  _fluxF(output,input);
  return true;
}
...
template<typename T, template<typename U> class DESCRIPTOR>
bool SuperLatticePhysVelocityFlux3D<T, DESCRIPTOR>::operator()(T output[], const int input[])
{
  _fluxF(output,input);
  return true;
}
