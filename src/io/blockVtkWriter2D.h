 ...
 * call write(BlockLatticeF2D<T,DESCRIPTOR>& f, int iT).
 *
...
#include "functors/blockLatticeCalcF2D.h"     // BlockLatticeIdentity
...
private:
  ...
  void dataArray(std::string& fullNameVti, int nx, int ny);
  void dataArrayBinary(std::string& fullNameVti, int nx, int ny);
...
private:
  ...
  std::string _name;
  std::vector< BlockLatticeIdentity2D<T,DESCRIPTOR>* > _pointerVec;
  int _offset;
  bool _binary;
};
